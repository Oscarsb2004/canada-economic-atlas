...
from atlas.shells.acquire import arcgis_layer, site_crawl
...
def projects_query_url(lang: str = "en", layer: int = LAYER_PROJECTS) -> str:
    ...
    return arcgis_layer.geojson_query_url(ARCGIS_BASE.format(lang=lang), layer)
...
def strategies_query_url(lang: str = "en") -> str:
    ...
    """
    return arcgis_layer.attributes_query_url(ARCGIS_BASE.format(lang=lang), LAYER_STRATEGIES)
...
_SAFE_SLUG = site_crawl.SAFE_SLUG


def crawl_section(fetch, section_path: str, *, max_pages: int = 300) -> dict:
    """Walk the MPO's canada.ca section; see `site_crawl.crawl_section`."""
    return site_crawl.crawl_section(fetch, CANADA_CA, section_path, max_pages=max_pages)
