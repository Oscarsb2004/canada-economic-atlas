...
| **B2a** | **Revenue by industry has no source yet.** `33100225` is *Quarterly balance sheet, income statement and selected financial ratios, by non-financial industries, non seasonally adjusted* — no finance sector, enterprise-based, a methodology break in Q1 2020, and only 5 of the 20 sectors (11, 22, 23, 54, 56) appear as a group of their own — the other 15 are split across enterprise groups or merged (education with health care, arts with accommodation). Decide: (a) find gross output by industry on the GDP basis (the supply and use tables are the likely home; table id unconfirmed), (b) a hand-curated, `DERIVED` crosswalk of its enterprise groups onto the 20 sectors, excluding finance, or (c) drop the revenue view. Blocks D5 and Q5. | G3 | S | ✓ |
...
| D5 | **Gross output vs value added by sector.** Blocked on B2a — the table declared for it is not revenue by industry. Makes the companies-panel caveat *visible* rather than a footnote — you could show directly why summing company revenues overshoots sector GDP two- to threefold. | G1 G3 | M | |
...
| Q5 | **Gross output against value added for finance.** Confirmed 2026-09-10 that 33100225 does NOT cover NAICS 52 — it is non-financial industries only — so this needs a different source (B2a). A bank's revenue is not a manufacturer's in any case. | G3 | M | |
