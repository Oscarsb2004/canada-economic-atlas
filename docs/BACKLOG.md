...
| C2 | ~~**Capital values from NRCan's Major Projects Inventory,** through the join C1 already built.~~ **Done 2026-09-13.** Stage 06 reads each joined project's "Capital cost ($M)" / "Dépenses en immobilisations (M$)" through the same declared join, never a second one, from **NRCan's open map service** — English and French layers, both resources of the open.canada.ca record under the Open Government Licence – Canada. The XLSX workbooks were read first and dropped the same day on the owner's decision: their caveats sheet says they are "not authorized or approved for publication at this time". The service publishes the same cost for every joined project it lists, but has no prior-year figures, only "Planned" / "Under Construction" as status, and does not list the Deep Geological Repository. A figure must be the same number in both languages. **9 of 18 projects carry a cost.** NRCan's disclaimer is read from the record, never retyped. | G2 G3 | S | |
| C3 | **Done 2026-09-13:** the project viewer has a "Capital cost" section — the service's figure under its own label ("Capital cost ($M): 20,900" for Darlington), "not published" otherwise, a line counted from the data on every project ("NRCan publishes a cost for 9 of the 18 projects. The atlas never adds them up…"), NRCan's disclaimer folded underneath, and the source linked. No total anywhere. ~~**State the ceiling in the UI.**~~ 9 of the 18 projects join the inventory's open map service (2026-09-13); the other nine have no cost there — none of the Transport or Industrial projects, not NCTL or Nukkiksautiit, and not the Deep Geological Repository. Show "not published" where there is no figure and **never total across the partial set**. A "$X billion portfolio" headline from ten of eighteen projects is wrong by construction, and this item is what stops C2 becoming that. | G1 | S | |
| C4 | ~~**Pin count and capital by NAICS sector, beside that sector's GDP.**~~ **Done 2026-09-13**, with one change from this row: capital is **listed per project, never summed per sector** — a sector total over the projects that have a cost would be the partial-set total C3 forbids. Under "One sector in context", a derived block lists the Major Projects Office projects placed in the chosen sector, each with NRCan's cost or "cost not published", counts projects and those with a cost, and states the sector's latest real GDP (chained 2017 dollars) beside them — side by side, never added or divided. A table covers every sector in the cube's order. Projects counted in construction are marked, and the table says why its Projects column does not add up to 18. Checked in a browser: Transportation and warehousing, 8 projects, 2 with a cost, real GDP 107,405 ($M, 2026-06); Construction, 3 projects, all with a cost. | G2 G3 | M | |
...
| | **Read 2026-09-13, before building (R0, R2, R3).** **R0:** Canadian Heritage has one page per jurisdiction under `canada.ca/en/canadian-heritage/services/provincial-territorial-symbols-canada/<name>.html`, each with its French twin under `/fr/patrimoine-canadien/services/symboles-provinciaux-territoriaux-canada/` (French names differ, e.g. `colombie-britannique`, `terre-neuve-et-labrador`); sections carry stable ids — Motto `#a4`, Flag `#a5` — and parse cleanly in both languages (Ontario checked). Flag terms, all 13 read 2026-09-13 — three clear permissions, the rest not: **Quebec** — images "peuvent être reproduites librement dans le respect de la loi", never for advertising, as a logo, or implying state authority (quebec.ca, updated 2025-09-05). **Alberta** — the Emblems of Alberta Act lets a person reproduce an official emblem other than the armorial bearings, conforming to the government's drawings. **Nova Scotia** — novascotia.ca materials may be reproduced for non-commercial purposes if credited, unmodified and not represented as official. **Ontario** — ontario.ca content may be reproduced non-commercially if credited and unchanged, but its terms of use say "official symbols of the Ontario government" need written permission, and do not say whether the flag is one. **Manitoba** — permission needed for provincial symbols and emblems; official symbols are excluded from OpenMB. **Saskatchewan** — provincial emblems are protected and need permission. **Newfoundland and Labrador** — the shield needs the Registrar General's written permission; the flag page states no rule and reserves all rights. **Nunavut** — its Canadian Heraldic Authority register entry says the emblems may not be reproduced without written consent. **British Columbia** — flag use is "not governed by legislation" but by convention; image requests go to its Office of Protocol. **New Brunswick, Prince Edward Island, Yukon, Northwest Territories** — no reproduction rule found. Under R0's rule, only the three with a clear permission are drawn; the other ten are described in Canadian Heritage's words until permission exists. **R2:** 36-10-0400 has 13 geographies and 26 industry members (aggregates included), 1997–2025, released 2026-05-01. **R3:** the November 2025 tables, Tables 18–30, one sheet per jurisdiction in both files. Every sheet carries the same eight columns — own-source revenues, federal transfers, total revenues, program expenditures, debt charges, total expenditures, deficit (-) or surplus, net debt — but Alberta's last column is "Net financial debt / assets (-)", and several add their own: "Other", stabilization or debt-retirement fund transfers, Quebec's Generations Fund, a reported balance. Provinces 1990-91 to 2024-25; Yukon and NWT to 2023-24; Nunavut from 1999-00. Each sheet ends with its own comparability notes (Ontario: expanded reporting from 2005-06, PSAB from 1993-94), which must travel with the series. English and French agree to within rounding — at most 0.5 ($M), Yukon. One label does not: the French Manitoba table (24) calls its 2011-12 row "2010-2011", so that year appears twice, while every figure on the row matches the English 2011-12. So the languages are paired by row with every figure checked, the English year is the key, and the typo is recorded rather than trusted. The French file writes years as "1990-1991". The index page lists no editions, so the newest edition must be found another way. | | | |
| | **Built 2026-09-13 as stage 08 — the data for R0, R2, R3 and R5a.** `data/provinces/provinces.json`: the Fiscal Reference Tables (2025 edition) for all 13, every published column and note in both languages, the French Manitoba label recorded; 36-10-0400 shares for the twenty sectors and All industries, 1997–2025; Canadian Heritage's motto and flag description; flag and coat of arms from Wikimedia Commons with each file's licence — **decided 2026-09-13 by the owner, replacing R0's "each government's flag rules": the images are public-domain or openly licensed Commons files, credited per file.** Verify gates the fiscal columns, the shares' total, both languages, and every CC BY-SA image's artist. Still open: R1 slogans, R5 budget text, R4 overview figures, and the province page itself (R6). | | | |
| | **R6 built 2026-09-13** (`web/src/panels/ProvincePanel.tsx`): flag, name, motto and the five largest sectors on the first screen; the latest fiscal year's published columns, deficit-or-surplus bars and a net debt line over every year, a table twin of every column, the table's notes and any French year-label mismatch; coat of arms, flag description and each image's Commons credit, with the insignia notice where Commons gives one. Two columns are charted by their English headings — "Deficit…" and "Net…" (Alberta's is net financial debt). Figures are formatted to two decimals by locale, which also hides the workbook's float noise. Checked in a browser for British Columbia (2024-25 deficit −7,347, net debt 87,306 $M; real estate 19.76 % of GDP in 2025). Still open: R1 slogans, R4 overview figures, R5 budget text. | | | |
