...
const en = {
  ...
  provinceFlagAlt: (name: string) => `Flag of ${name}`,
  provinceArmsAlt: (name: string) => `Coat of arms of ${name}`,
  provinceNoProfile: "No profile is published for this province or territory.",
  provinceSectorsHeading: (year: string) => `Largest sectors, ${year}`,
  provinceSectorsNote:
    "Statistics Canada's own percentage shares of GDP at basic prices, current dollars, listed largest first. Real estate includes owner-occupied dwellings.",
  provinceSectorsCaption: (year: string) => `Every sector's share of GDP, ${year} (Statistics Canada)`,
  colShare: "Share of GDP (%)",
  provinceIdentityHeading: "Flag and coat of arms",
  provinceFlagSource: "Flag description: Canadian Heritage, Provincial and territorial symbols",
  provinceImageCredit: (kind: string, title: string, licence: string, artist: string) =>
    `${kind}: ${title} · ${licence}${artist ? ` · ${artist}` : ""} · Wikimedia Commons`,
  provinceFlagWord: "Flag",
  provinceArmsWord: "Coat of arms",
  provinceInsignia: "Commons marks these as insignia: their use may be restricted by law, whatever the licence.",
  provinceFinanceHeading: (year: string) => `Public finances, ${year}`,
  provinceFinanceUnit: "Millions of dollars, Public Accounts basis, as Finance Canada's Fiscal Reference Tables publish them",
  provinceFinanceCaption: (name: string) => `${name}: every published column, every fiscal year (millions of dollars)`,
  provinceBalanceFigure: (label: string) => `${label}, every fiscal year`,
  provinceDebtFigure: (label: string) => `${label}, every fiscal year`,
  provinceFinanceNotes: "Finance Canada's notes on this table — where the series changes",
  provinceYearMismatch: (en: string, fr: string) =>
...
const fr: Strings = {
  ...
  provinceFlagAlt: (name) => `Drapeau : ${name}`,
  provinceArmsAlt: (name) => `Armoiries : ${name}`,
  provinceNoProfile: "Aucun profil n’est publié pour cette province ou ce territoire.",
  provinceSectorsHeading: (year) => `Principaux secteurs, ${year}`,
  provinceSectorsNote:
    "Parts en pourcentage du PIB aux prix de base, en dollars courants, publiées par Statistique Canada, du plus grand au plus petit. Les services immobiliers comprennent les logements occupés par leur propriétaire.",
  provinceSectorsCaption: (year) => `Part du PIB de chaque secteur, ${year} (Statistique Canada)`,
  colShare: "Part du PIB (%)",
  provinceIdentityHeading: "Drapeau et armoiries",
  provinceFlagSource: "Description du drapeau : Patrimoine canadien, Symboles provinciaux et territoriaux",
  provinceImageCredit: (kind, title, licence, artist) =>
    `${kind} : ${title} · ${licence}${artist ? ` · ${artist}` : ""} · Wikimedia Commons`,
  provinceFlagWord: "Drapeau",
  provinceArmsWord: "Armoiries",
  provinceInsignia: "Commons les signale comme insignes : leur usage peut être restreint par la loi, quelle que soit la licence.",
  provinceFinanceHeading: (year) => `Finances publiques, ${year}`,
  provinceFinanceUnit: "Millions de dollars, selon les comptes publics, tels que les publient les Tableaux de référence financiers du ministère des Finances",
  provinceFinanceCaption: (name) => `${name} : chaque colonne publiée, chaque exercice (millions de dollars)`,
  provinceBalanceFigure: (label) => `${label}, chaque exercice`,
  provinceDebtFigure: (label) => `${label}, chaque exercice`,
  provinceFinanceNotes: "Notes du ministère des Finances sur ce tableau — là où la série change",
  provinceYearMismatch: (en, fr) =>
